...
npm install -D vite@8.0.3 typescript@6.0.2

# Database & offline storage
npm install dexie@4.4.1 dexie-react-hooks@4.4.0

# PWA
npm install -D vite-plugin-pwa@1.2.0 @vite-pwa/assets-generator@1.0.2
...
# Testing
npm install -D vitest@4.1.2 @testing-library/react@16.3.2 @testing-library/user-event@14.6.1 jsdom@25.0.1
...
**If building a multi-user sync app:**
...
For storing images in IndexedDB (as required by Binkashi):

**Dexie supports Blob storage natively:**
```typescript
db.version(1).stores({
  bins: '++id, name, state, startDate, endDate'
})
...
await db.bins.add({
  ...
  state: 'inUse',
  startDate: new Date(),
  image: file // File/Blob object - Dexie handles this
})
```
...
1. User action → Update UI optimistically
2. Write to IndexedDB immediately (Dexie.js)
3. If online, sync to backend (not needed for Binkashi - single-user)
4. Use `useLiveQuery()` to observe IndexedDB and re-render automatically

### Manifest Configuration
Required for mobile installability:
...
---
*Stack research for: React PWA Offline-First App*
*Researched: 2025-03-29*
